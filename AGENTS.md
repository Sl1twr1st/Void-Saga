# VOID SAGA — Narrative Operating System (AGENTS.md)
...
Developer Zero (complete): `docs/30_MINUTE_CHALLENGE.md` — a checkpoint-based tutorial using a non–Void Saga character (Mira / The Last Cup). Target: someone unfamiliar with Void Saga builds a runtime without author assistance.

Scene Triaging MVP (complete): `scripts/triage-scenes` — batch classifier. Writer writes 3-5 scene ideas as 10-line JSON, runs triage, gets PASS/WARNING/BLOCKED table. Compiler becomes the first editor — not a generator, a decision tool.

Narrative Studio (complete): Four-layer workspace — Canon (Bab/Timer), Decision (dogfood/ + compiler), Drafting (writing/), Publication. Distinct rooms for activities that were mixed together for two years.
...
Project ini sudah melewati fase "Jali nulis chapter → Codex baca → Codex kasih feedback."

Mode operasi sekarang:
...
1. **Read CLAUDE.md first.** Ini adalah handoff document. Future Codex sessions harus mulai dari sini.
...
3. **Respect the pipeline.** Jika bekerja di compiler/engine/runtime: pahami arsitektur dulu sebelum mengubah. Pipeline sudah proven dengan live generation — jangan break yang sudah bekerja.
4. **Novel voice is Jali's.** Codex tidak menulis ulang prosa novel kecuali diminta eksplisit.
...
| `scripts/triage-scenes` | Batch classifier — PASS/WARNING/BLOCKED |
