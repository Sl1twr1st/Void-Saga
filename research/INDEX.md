...
| 2026-07-17 | **Provenance loss trace** — canon identity never enters the engine-side pipeline; loss is a missing edge, not a leak | First M2 investigation ("di titik mana citation kehilangan identitas kanonnya?"). Grep `document_id\|source_hash` across `VOID_SAGA_UNIVERSE/` = 0 hits. 122 citation strings in 8 runtimes: 43 informal labels / 10 fuzzy / 28 path-based (mostly non-canon targets) / 41 pure prose. `forbidden_behaviors` (hard-block drivers, 42 entries) carry no citation field at all. Internal loss point: `constraints.py:402-412` drops even free-text evidence from violations. | ⏳ `research/2026-07-17-provenance-loss-trace.md`. Design decision (citation identity's home: schema vs resolver-side mapping) deliberately NOT taken. |
| 2026-07-17 | **Citation residence verdict: A — Promote provenance into the Evidence Engine contract.** Evidence tanpa provenance tidak lagi sah; implementation strategy OPEN | Decision matrix (authority / drift risk / migration cost / B3 survivability) atas temuan provenance trace. B ditolak karena topologi, bukan drift: sidecar menjadikan resolver — bukan engine — sebagai consumer canon; matikan resolver → engine tetap PASS → melanggar exit criterion M2. Bukan "schema change accepted" — schema hanyalah konsekuensi yang mungkin; standing rule "Do NOT change schema" tetap berlaku sampai implementation strategy dipilih. | ✅ Governs M2 contract. `research/2026-07-17-citation-residence-decision.md`. First subsystem contract change driven by evidence chain: spike → trace → matrix → verdict. |
